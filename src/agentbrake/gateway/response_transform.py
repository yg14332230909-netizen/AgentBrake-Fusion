...
def transform_response(
    ...
    if blocked:
        message = safe_block_message("AgentBrake Gateway policy result:", blocked, trace_id)
    ...
    if sandbox_only and release_mode != "gateway_plus_guarded_tools":
        message = safe_sandbox_only_message("AgentBrake Gateway sandbox-only result:", sandbox_only, trace_id)
        return assistant_message_response(message, model=model)
