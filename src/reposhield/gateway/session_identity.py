...
def resolve_session_identity(
    *,
    ...
    explicit = metadata.get("reposhield_run_id")
    if explicit:
        return SessionIdentity(str(explicit), conversation_id, turn_id, client_id, task_id, "metadata.reposhield_run_id")
    ...
    header_run = _header(headers, "X-RepoShield-Run-Id")
    if header_run:
        return SessionIdentity(header_run, conversation_id, turn_id, client_id, task_id, "header.x-reposhield-run-id")
