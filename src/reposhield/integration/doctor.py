...
def run_doctor(repo_root: str | Path, *, config_path: str | Path | None = None, agent: str | None = None) -> DoctorReport:
    ...
    if not config_file.exists():
        return DoctorReport(
            ...
            {},
            warnings,
            ["reposhield connect --repo . --agent custom-openai --mode quick"],
        )
...
def _repair_hint(name: str, ok: bool, detail: str) -> str:
    ...
    hints = {
        ...
        "agent_protocol_declared": "Regenerate config for this agent: reposhield connect --repo . --agent <agent> --force",
        "agent_authorization_declared": "Regenerate config: reposhield connect --repo . --force",
        ...
        "gateway_chat_smoke": "Start Gateway and verify /v1/chat/completions accepts Authorization: Bearer reposhield-local.",
        "gateway_responses_smoke": "Start Gateway and verify /v1/responses accepts Authorization: Bearer reposhield-local.",
...
def run_smoke_test(config: dict[str, Any], agent_profile: AgentProfile | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        with urlopen(request, timeout=2) as response:
            ok = response.status == 200 and bool(response.headers.get("X-RepoShield-Run-Id"))
            return {"ok": ok, "endpoint": endpoint, "status": response.status, "run_id_header": response.headers.get("X-RepoShield-Run-Id")}
...
def _smoke_payload(profile: AgentProfile, session: dict[str, Any]) -> dict[str, Any]:
    metadata = {
        ...
        "client_id": f"reposhield-doctor-{profile.agent}",
    }
...
def _next_commands(config: dict[str, Any], profile: AgentProfile, checks: list[dict[str, Any]]) -> list[str]:
    ...
    if "agent_protocol_declared" in names or "agent_authorization_declared" in names or "stable_run_identity" in names:
        commands.append(f"reposhield connect --repo . --agent {profile.agent} --mode {config.get('mode', 'quick')} --force")
    if "gateway_port_listening" in names:
        commands.append("reposhield start --repo . --gateway-only")
    if "studio_port_listening" in names and config.get("studio", {}).get("enabled"):
        commands.append("reposhield start --repo .")
    if "shims_on_path" in names:
        commands.append('export PATH="$(pwd)/.reposhield/shims:$PATH"')
    if any(name.endswith("_smoke") for name in names):
        commands.append(f"reposhield smoke-test --repo . --agent {profile.agent}")
