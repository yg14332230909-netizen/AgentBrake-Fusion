...
class AgentProfile:
    ...
    protocol: str
    wire_api: str
    model: str = "reposhield/local-heuristic"
    api_key_env: str = "REPOSHIELD_GATEWAY_API_KEY"
    api_key_value: str = "reposhield-local"
...
def _load_yaml_profiles() -> dict[str, AgentProfile]:
    try:
        ...
        root = files("reposhield.integration").joinpath("profiles")
...
def _profile_from_mapping(data: dict[str, Any]) -> AgentProfile:
    return AgentProfile(
        ...
        model=str(data.get("model") or "reposhield/local-heuristic"),
        api_key_env=str(data.get("api_key_env") or "REPOSHIELD_GATEWAY_API_KEY"),
        api_key_value=str(data.get("api_key_value") or "reposhield-local"),
...
def _fallback_profiles() -> dict[str, AgentProfile]:
    return {
        "generic": AgentProfile(
            ...
            wire_api="chat",
            config_files=(".reposhield/agent.env", ".reposhield/agent-instructions.md"),
            notes=("Use OPENAI_BASE_URL and OPENAI_API_KEY from .reposhield/agent.env.",),
        ),
        "custom-openai": AgentProfile(
            ...
            wire_api="chat",
            config_files=(".reposhield/agent.env", ".reposhield/agent-instructions.md"),
            notes=("Set the agent model provider base URL to RepoShield Gateway.",),
        ),
        "codex": AgentProfile(
            agent="codex",
            ...
            protocol="openai-compatible",
            wire_api="responses",
            api_key_env="REPOSHIELD_GATEWAY_API_KEY",
        ...
        "claude-code": AgentProfile(
            ...
            protocol="openai-compatible",
            wire_api="chat",
            config_files=(".reposhield/agent.env",),
        ),
        "cursor": AgentProfile(
            agent="cursor",
