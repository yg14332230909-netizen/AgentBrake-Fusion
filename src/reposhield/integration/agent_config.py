...
def apply_agent_config(repo: Path, config: dict[str, Any], profile: AgentProfile, *, force: bool = False) -> AgentConfigChange:
    ...
    backup = repo / ".reposhield" / "backups" / "codex-config.toml.bak"
...
def _write_snippet(repo: Path, config: dict[str, Any], profile: AgentProfile, *, force: bool) -> AgentConfigChange:
    target = repo / ".reposhield" / "agent-config" / f"{profile.agent}.env"
    backup = repo / ".reposhield" / "backups" / f"{profile.agent}.env.bak"
    target.parent.mkdir(parents=True, exist_ok=True)
    backup.parent.mkdir(parents=True, exist_ok=True)
...
def _render_codex_config(config: dict[str, Any], original: str) -> str:
    ...
    block = "\n".join(
        [
            ...
            'name = "RepoShield Gateway"',
            f'base_url = "{gateway["base_url"]}"',
            'wire_api = "responses"',
            'env_key = "REPOSHIELD_GATEWAY_API_KEY"',
            f'http_headers = {{ X-RepoShield-Run-Id = "{session["run_id"]}" }}',
            "# END RepoShield managed block",
            "",
        ]
    )
...
def _render_openai_env(config: dict[str, Any], profile: AgentProfile) -> str:
    ...
    return "\n".join(
        [
            "# RepoShield generated agent config snippet.",
            f"OPENAI_BASE_URL={gateway['base_url']}",
            f"{profile.api_key_env}={profile.api_key_value}",
            f"REPOSHIELD_RUN_ID={session['run_id']}",
            f"REPOSHIELD_CONVERSATION_ID={session['conversation_id']}",
            "",
        ]
    )
...
def _remove_managed_block(text: str) -> str:
    start = "# BEGIN RepoShield managed block"
    end = "# END RepoShield managed block"
