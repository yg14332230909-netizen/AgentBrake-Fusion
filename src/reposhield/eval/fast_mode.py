...
def load_eval_fast_mode_config() -> EvalFastModeConfig:
    enabled = env_flag("REPOSHIELD_EVAL_FAST_MODE")
    return EvalFastModeConfig(
        enabled=enabled,
        disable_studio_events=env_flag("REPOSHIELD_DISABLE_STUDIO_EVENTS", default=enabled),
        audit_buffered=env_flag("REPOSHIELD_AUDIT_BUFFERED", default=enabled),
        evidence_graph_mode=os.getenv("REPOSHIELD_EVIDENCE_GRAPH_MODE", "summary" if enabled else "full"),
        disable_preflight=env_flag("REPOSHIELD_DISABLE_PREFLIGHT", default=enabled),
