...
def _default_invariants() -> list[RegisteredInvariant]:
    return [
        RegisteredInvariant(
            ...
            lambda f: f.any_value("asset", "touched_type", {"secret_file", "secret_env", "publish_config", "forbidden_file"}) or any(x.value is True for x in f.find("contract", "forbidden_file_touch")),
        ...
        RegisteredInvariant(
            ...
            lambda f: _bool(f, "source", "has_untrusted") and _bool(f, "action", "high_risk") and f.any_value("contract", "match", {"violation", "unknown"}),
            lambda f: f.find("source") + f.find("action", "high_risk") + f.find("contract", "match"),
        ),
        RegisteredInvariant(
            SemanticInvariantSpec("INV-REPO-001", "repo_boundary_escape", "asset.repo_escape OR asset.symlink_escape", ["asset.repo_escape", "asset.symlink_escape"], DecisionConstraints(execution_env="none", audit_scope="full"), ["repo_escape_or_symlink_escape"], ["block"], 100, "repo_boundary"),
        ...
        RegisteredInvariant(
            SemanticInvariantSpec("INV-CI-001", "untrusted_ci_modification", "asset.touched_type=ci_workflow AND source.has_untrusted", ["asset.touched_type", "source.has_untrusted"], DecisionConstraints(execution_env="none", human_gate="approval_required", audit_scope="full"), ["untrusted_source_cannot_modify_ci_asset"], ["block", "human_approval"], 95, "ci"),
        ...
        RegisteredInvariant(
            SemanticInvariantSpec("INV-SC-001", "untrusted_remote_dependency", "package.source IN {git_url,tarball_url} AND source.has_untrusted", ["package.source", "source.has_untrusted"], DecisionConstraints(execution_env="none", network_scope="deny", audit_scope="full"), ["untrusted_source_cannot_authorize_remote_package"], ["block", "package_preflight"], 95, "supply_chain"),
        ...
        RegisteredInvariant(
            SemanticInvariantSpec("INV-MCP-001", "destructive_mcp_capability", "mcp.decision=blocked OR destructive mcp capability", ["mcp.decision", "mcp.capability"], DecisionConstraints(execution_env="none", audit_scope="full"), ["destructive_or_blocked_mcp_tool"], ["mcp_proxy", "block"], 92, "mcp"),
            lambda f: f.any_value("mcp", "decision", {"blocked"}) or any(str(x.value).lower() in {"invoke_destructive_mcp_tool", "deploy", "publish", "delete", "auth", "credential"} for x in f.find("mcp", "capability")),
            lambda f: f.find("mcp"),
        ),
        RegisteredInvariant(
            SemanticInvariantSpec("INV-MEM-001", "tainted_memory_authorization", "memory.authorization_denied AND action.high_risk", ["memory.authorization_denied", "action.high_risk"], DecisionConstraints(execution_env="none", audit_scope="full"), ["memory_authorization_denied_high_risk"], ["memory_taint_gate", "block"], 92, "memory"),
            lambda f: _bool(f, "memory", "authorization_denied") and _bool(f, "action", "high_risk"),
            lambda f: f.find("memory") + f.find("action", "high_risk"),
        ),
        RegisteredInvariant(
            SemanticInvariantSpec("INV-PARSER-001", "low_confidence_side_effect", "action.parser_confidence<0.6 AND action.side_effect", ["action.parser_confidence", "action.side_effect"], DecisionConstraints(execution_env="sandbox", human_gate="approval_required", audit_scope="full"), ["parser_confidence_below_threshold"], ["sandbox_preflight", "human_approval"], 82, "parser"),
            lambda f: any(isinstance(x.value, (int, float)) and x.value < 0.6 for x in f.find("action", "parser_confidence")) and _bool(f, "action", "side_effect"),
...
def _hit(spec: SemanticInvariantSpec, evidence_facts: list[PolicyFact]) -> RuleHit:
    ...
    predicates = [
        {"fact_id": fact.fact_id, "namespace": fact.namespace, "key": fact.key, "value": fact.value, "matched": True, "semantic_invariant": spec.invariant_id}
...
def _secret_event(facts: PolicyFactSet) -> bool:
    return facts.any_value("secret", "event", {"egress_after_secret", "token_like_value_in_tool_args", "secret_read"}) or bool(facts.find("secret", "event"))
