...
def cmd_smoke_test(args: argparse.Namespace) -> int:
    config_path = Path(args.config).resolve() if args.config else Path(args.repo).resolve() / ".reposhield" / "config.yaml"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    connect = sub.add_parser("connect", help="Generate formal RepoShield agent integration files")
    ...
    connect.add_argument("--apply-agent-config", action="store_true", help="Apply a supported host-agent config with a RepoShield backup.")
    connect.add_argument(
        "--restore-agent-config", action="store_true", help="Restore the host-agent config from the RepoShield backup manifest."
    )
    connect.set_defaults(func=cmd_connect)

    start = sub.add_parser("start", help="Start RepoShield services generated by reposhield connect")
    ...
    status = sub.add_parser("status", help="Show RepoShield background service status for this repo")
    status.add_argument("--repo", default=".")
    status.add_argument("--config")
    status.set_defaults(func=cmd_status)

    stop = sub.add_parser("stop", help="Stop RepoShield background services started for this repo")
    ...
    doctor = sub.add_parser("doctor", help="Validate RepoShield formal agent integration readiness")
