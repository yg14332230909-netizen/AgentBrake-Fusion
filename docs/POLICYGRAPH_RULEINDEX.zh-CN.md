...
这篇文档说明 AgentBrake 当前最核心的技术点：如何把 coding agent 的多源证据综合成可解释的安全决策。
...
- `AGENTBRAKE_ENABLE_ACTION_GRAPH`、`AGENTBRAKE_ENABLE_SESSION_STATE`、`AGENTBRAKE_ENABLE_CONSTRAINT_LATTICE` 可用于回退到兼容行为。
...
这块最适合用来解释项目的核心创新点：AgentBrake 不是简单拦截器，而是可观测、可解释、可审计的多源证据融合决策系统。
