# AgentBrake 正式智能体接入指南

本文档对应 `agentbrake connect / start / status / stop / doctor / smoke-test / coverage / profiles / integration-matrix`，用于把真实 coding agent 低摩擦接入 AgentBrake。

## 三种接入模式

| 模式 | 适用场景 | 生成内容 |
| --- | --- | --- |
| Quick | 只需要 OpenAI-compatible Gateway 治理模型边界 | `.agentbrake/config.yaml`、`agent.env`、agent instructions、Gateway 启动脚本 |
| Standard | 需要 Gateway + 常见 shell/file 工具治理 | Quick 内容 + `.agentbrake/shims/*` 工具 shim |
...
新增或切换智能体时，差异必须优先进入 `src/agentbrake/integration/profiles/*.yaml`：
...
默认 `connect` 只生成仓库内 `.agentbrake` 文件，不修改真实智能体配置。
...
Codex 当前是 `native` 写入；其他 OpenAI-compatible 智能体会生成 `.agentbrake/agent-config/<agent>.env` 片段，供用户或上层工具接入。

## Smoke Test

只探测 AgentBrake Gateway：
...
```text
X-AgentBrake-Run-Id: run_xxxxxxxxxxxxxxxx
```

不要在同一任务中重新生成这些值。否则 AgentBrake 无法把 PersistentSessionState、ActionGraph、secret taint、依赖风险和审批约束连续起来。
...
{
  "next_commands": [
    "agentbrake connect --repo . --agent codex --mode full --force",
    "agentbrake start --repo . --gateway-only",
    "agentbrake smoke-test --repo . --agent codex"
  ]
}
```
...
1. 网关侧启用 `AGENTBRAKE_EVAL_FAST_MODE=1`，压缩 `EvidenceGraph`、审计刷新和 preflight。
2. 工具执行前使用 `AgentBrakeToolGate`，把工具分类映射为 `agentdojo.*` facts，再交给 `PolicyEngine`。
