...
## 正式智能体接入：Quick / Standard / Full

现在可以用统一命令生成真实 agent 接入文件：
...
多轮 agent 每一轮都必须传入稳定 `metadata.reposhield_run_id` 和 `metadata.conversation_id`。Gateway 也会返回 `X-RepoShield-Run-Id`，便于客户端确认本轮解析到的 run_id。
