...
# AgentBrake AgentDojo ToolGate

这个实验不是 Gateway-only。Gateway 只能看模型响应链路，ToolGate 才是每个 AgentDojo tool call 真正执行前的拦截点。

## 关键点

- `Gateway-only`：模型响应先过 AgentBrake Gateway，再由 AgentDojo 执行工具
- `ToolGate`：AgentDojo 的每一次 tool call 在 `FunctionsRuntime.run_function()` 前先过 AgentBrake
...
Gateway-only 只能影响模型输出，不足以约束 AgentDojo 里后续的工具执行。AgentBrake 要真正评测工具防御能力，必须在 `runtime.run_function()` 之前做判断。
...
- AgentDojo 本身不支持自定义 defense registry，AgentBrake 用 pipeline wrapper 接入
